...
def test_cancel_product_orders_params(senders: list[str]):
    ...
    params_from_obj = CancelProductOrdersParams(sender=sender_1, productIds=product_ids)
    bytes32_sender = CancelProductOrdersParams(
        sender=hex_to_bytes32(sender_1), productIds=product_ids
    )

    assert params_from_dict == params_from_obj == bytes32_sender
    # req_from_params = CancelProductOrdersRequest()
